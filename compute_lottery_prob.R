# install.packages(c("dplyr","magrittr","tidyr","ggplot2"))
...
lottery_df <- read.csv("lottery_df.csv")
...
long.format.df %>% select(Date,X) %>% mutate(N = 1) %>% spread(key = X, value = N, fill=0) %>% View
...
jump_results %>% as.data.frame %>% ggplot(.) + geom_bar(mapping = aes(x=reorder(jump,-wp),y=wp),stat="identity")
...
all_ngram_sum_df %>% arrange(i) %>% View

wide_sum_all_ngram_sum_df = all_ngram_sum_df %>% select(i,k,ngram_sum) %>% spread(key=k,value=ngram_sum) 

names(wide_sum_all_ngram_sum_df) <- c("i",paste0("X",2:50))
names(wide_sum_all_ngram_sum_df)

wide_sum_all_ngram_sum_df %>% group_by(X2,X3) %>% summarise(n=n()) %>% arrange(-n) %>% View
wide_sum_all_ngram_sum_df %>% group_by(X3,X4) %>% summarise(n=n()) %>% arrange(-n) %>% View
wide_sum_all_ngram_sum_df %>% group_by(X2,X4) %>% summarise(n=n()) %>% arrange(-n) %>% View
